...
import 'package:vnt_app/pages/intranet_access_page.dart';
...
/// 主导航框�?- 响应式布局，支持侧边栏和底部导�?class MainNavigationShell extends StatefulWidget {
...
class _MainNavigationShellState extends State<MainNavigationShell> {
  ...
  Future<void> _autoConnect() async {
    ...
    if (Platform.isAndroid) {
      try {
        ...
        debugPrint('检查磁贴启动状态失�? $e');
      }
    }

    // 如果不是从磁贴启动，检查自动连接设�?    if (!isTileStart) {
    ...
    // 如果没有磁贴配置key，使用默认配�?    if (targetKey == null || targetKey.isEmpty) {
      ...
      if (msg is String) {
        if (msg == 'success') {
          if (onece) {
            ...
            // 连接成功，更新磁贴和小组件状�?            if (Platform.isAndroid) {
          ...
          // 统一显示"服务已停�?提示
          showTopToast(context, '[${config.configName}] 服务已停�?, isSuccess: false);
          // 服务停止，更新磁贴和小组件状�?          if (Platform.isAndroid) {
        ...
        // 连接错误，更新磁贴和小组件状�?        if (Platform.isAndroid) {
          VntAppCall.updateWidgetAndTile(vntManager.hasConnection());
        }
        // 更新系统托盘
        ...
        if (onece && msg.count > BigInt.from(60)) {
          ...
          // 连接超时，更新磁贴和小组件状�?          if (Platform.isAndroid) {
            VntAppCall.updateWidgetAndTile(vntManager.hasConnection());
          }
          // 更新系统托盘
  ...
  Widget _buildSideNavigation(bool isDark, bool isExpanded) {
    ...
    // 根据屏幕高度自动缩放所有尺寸，避免太小或太大出现滚�?    // 基准高度�?00px，高度越小缩放比例越小，高度越大缩放比例越大
    double heightScale;
    if (screenHeight >= 900) {
      heightScale = 1.2;  // 超大屏幕：放�?0%
    } else if (screenHeight >= 700) {
      heightScale = 1.0;  // 标准屏幕�?00%
    } else if (screenHeight >= 600) {
      heightScale = 0.85; // 中等屏幕：缩�?5%
    } else if (screenHeight >= 500) {
      heightScale = 0.75; // 小屏幕：缩小25%
    } else {
      heightScale = 0.65; // 超小屏幕：缩�?5%
    }
                  ...
                  ClipRRect(
                    ...
                    child: Image.asset(
                      ...
                      errorBuilder: (context, error, stackTrace) {
                        // 如果图片加载失败，显示默认图�?                        return Container(
                          width: logoSize,
                          height: logoSize,
                          decoration: BoxDecoration(
  ...
  Widget _buildPageContent() {
    ...
    // 使用 IndexedStack 保持页面状态，避免切换时重建页�?    return IndexedStack(
      ...
      children: [
        ...
        ConfigListPage(
          onConfigSelected: (config) {
            setState(() {
              _selectedConfig = config;
              _selectedIndex = 1; // 跳转到房间页�?            });
          },
          onRefreshCallback: (callback) {
            _refreshConfigList = callback;
          },
          onDataChanged: () {
            // 当配置数据改变时，刷新设置页�?            _refreshSettings?.call();
          },
        ),
        // 3: 内网访问
